...
- **Only optional single-token authentication; no authorization model.** When `SSR_MCP_AUTH_TOKEN` is unset (the default), any client that can reach the service can list and retrieve bundles, request install payloads, read audits, and create, edit, deprecate, or delete registry metadata. Setting the token requires `Authorization: Bearer` on the tool, registry-editing, and audit routes, but it is one shared secret with no accounts, roles, or per-client identity — every token holder has full admin authority, and the separate admin route names are still not an access-control boundary.
- **No built-in TLS.** Loopback prevents off-host clients from reaching the service by default; it does not authenticate same-host clients or protect a host that is already compromised. Without TLS, a configured bearer token crosses the network in plaintext and can be captured on an untrusted network. Cross-machine operators must supply a trusted private network or authenticated TLS proxy.
