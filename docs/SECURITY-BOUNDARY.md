...
- Install paths remain category/name descendants of the selected root and are validated along with bundle paths, frontmatter, and SHA-256 checksums before files are committed. The adapter stages a complete validated bundle and swaps it into place as a whole; `overwrite=true` removes files no longer present in the bundle instead of merging stale content forward, and a failed final swap attempts to restore the previous installation when possible.
